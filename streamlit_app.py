...
# ==========================================
# INDICADORES POR OPERADOR (NUEVO MÓDULO)
# ==========================================
st.markdown("---")
...
if not df_op_f.empty:
    ...
    df_op_f['Perf_Num'] = pd.to_numeric(df_op_f['Performance'], errors='coerce').fillna(0)
    if df_op_f['Perf_Num'].mean() <= 1.5 and df_op_f['Perf_Num'].mean() > 0:
        df_op_f['Perf_Num'] = df_op_f['Perf_Num'] * 100
        
    df_resumen = df_op_f.groupby(col_op).agg(
        Días=('Fecha_Filtro', 'nunique'),
        Perf_Promedio=('Perf_Num', 'mean')
    ).reset_index().sort_values('Perf_Promedio', ascending=False)
    
    if tipo_informe in ["Semanal", "Mensual"]:
        st.markdown("💡 **Seleccione un operario para visualizar su rendimiento diario contra el objetivo.**")
        
        c_izq, c_der = st.columns([1, 2])
        
        with c_izq:
            operario_sel = st.selectbox("📌 Seleccionar Operario:", df_resumen[col_op].tolist())
            st.dataframe(
                df_resumen.style.format({'Perf_Promedio': '{:.1f}%'})
                .background_gradient(subset=['Perf_Promedio'], cmap='RdYlGn', vmin=50, vmax=100),
                hide_index=True, 
        ...
        with c_der:
            if operario_sel:
                df_ind = df_op_f[df_op_f[col_op] == operario_sel].sort_values('Fecha_Filtro')
                df_ind['Fecha_Str'] = df_ind['Fecha_Filtro'].dt.strftime('%d-%b-%Y')
                
                fig_op = px.bar(
                    df_ind, x='Perf_Num', y='Fecha_Str', orientation='h', text='Perf_Num',
                    title=f"Evolución Diaria: {operario_sel}",
                    labels={'Perf_Num': 'Performance (%)', 'Fecha_Str': 'Día'}
                )
                ...
                fig_op.add_vline(x=90, line_dash="dash", line_color="black", annotation_text=" Meta 90%", annotation_position="top right")
                
                max_x = max(100, df_ind['Perf_Num'].max() * 1.15 if not df_ind.empty else 100)
                fig_op.update_xaxes(range=[0, max_x])
                fig_op.update_layout(yaxis={'categoryorder':'category descending'}, height=max(350, len(df_ind)*45))
                st.plotly_chart(fig_op, use_container_width=True)
                
    else:
        with st.expander("👉 Ver Resumen General de Operarios", expanded=True):
            st.dataframe(
                df_resumen.style.format({'Perf_Promedio': '{:.1f}%'})
                .background_gradient(subset=['Perf_Promedio'], cmap='RdYlGn', vmin=50, vmax=100),
                hide_index=True, 
...
# ==========================================
# RESTO DEL DASHBOARD (Tiempos, Producción, Fallas)
# ==========================================
